...
describe CalendarController do
  ...
  describe 'admin index action' do
    ...
    context 'invalid unit with valid shift' do
    ...
    context 'invalid unit without valid shift' do
      invalid_inputs = [nil,'53a',2343]
      invalid_inputs.each do |input|
        before :each do
          FactoryGirl.create(:unit)
    ...
    context 'invalid shift with valid unit' do
    ...
    context 'invalid shift with no unit' do
      invalid_inputs = [nil,'53a',2343,'apple']
      invalid_inputs.each do |input|
        before :each do
          unit = FactoryGirl.create(:unit)
    ...
    context 'invalid shift with invalid unit' do
      invalid_inputs = [nil,'53a',2343,'apple']
      invalid_inputs.each do |input|
        before :each do
          unit = FactoryGirl.create(:unit)
    ...
    context 'month or day is zero' do
  ...
  describe "Edit" do
    ...
    context 'Edit with invalid event id' do
      before :each do
  ...
  describe 'Finalize Vacation Schedule' do
    before :each do
      ...
      @baton = NurseBaton.create(:unit_id => @unit.id, :shift => @shift, :current_nurse_id => @nurse.id)
      post :finalize_schedule, :nurse_id => @nurse.id
    end
    it 'should make the current nurse the next nurse' do
      nurse = NurseBaton.find_by_unit_id_and_shift(@unit.id,@shift)
      nurse.current_nurse.should == @nurse_next
    end
  end
end
