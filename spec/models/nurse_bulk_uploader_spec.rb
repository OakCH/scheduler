...
describe NurseBulkUploader do
  ...
  describe 'checking for header row in spreadsheet' do
    ...
    context 'where header row is missing the email column' do
      before(:each) do
        @uploader.sheet = Excel.new path_helper("missing_email_header.xls") 
        @ret = @uploader.set_column_positions
      end
      ...
      subject { @uploader.parsing_errors }
      its ([:database_changed]) { should == false }
      its ([:messages]) { should include 'Header row is missing the Email column' } 
    end
  ...
  describe 'regular expression matching of header naming' do
    ...
    ['email', 'e-mail'].each do |term|
      it "should match #{term} to the email column" do
        @uploader.match_email(term).should_not be_nil
      end
    end
  end
  ...
  describe 'nice column name from hash symbol' do
    expected_nice_names = { :name => 'Name', :years_worked => 'Years Worked',
      :num_weeks_off => 'Num Weeks Off', :email => 'Email'}
  ...
  describe 'creating nurses' do
    ...
    context 'with one nurse' do
      before do
        ...
        @uploader.cols = {:name => 1, :years_worked => 2, :num_weeks_off => 3, :email => 4}
        @uploader.create_nurses
